...
class RandomBrain(Brain):
    ...
    :param output_space: the space of possible actions to sample from, satisfying the `Space` interface used by
        OpenAI Gym
    """
...
class KeyboardBrain(Brain):
    """
    A non-autonomous 'brain' that allows users to control an agent via the keyboard.
...
class PittRulesBrain(Brain):
    """
    A 'brain' that interprets a Pittsburgh-style ruleset and outputs the appropriate action.
    ...
    :param rules: a list of rules, each of the form `[ c1 c1'  c2 c2' ... cn cn'  a1 ... am m1 ... mr]`
    :param priority_metric: the rule prioritization strategy used to resolve conflicts

    Rulesets are lists of rules.  Rules are lists of the form `[ c1 c1'  c2 c2' ... cn cn'  a1 ... am m1 ... mr]`, where
    `(cx, cx')` are are the min and max bounds that the rule covers, `a1 .. am` are the output actions, and `m1 ... mr`
    are values to write to the memory registers.

    For example, this ruleset has two rules.  The first rule covers the square bounded by `(0.0, 0.6)' and `(0.0, 0.4)`,
    returning the output action `0` if the input falls within that range:

    >>> rules = [[0.0,0.6, 0.0,0.4, 0],
    ...          [0.4,1.0, 0.6,1.0, 1]]

    The input and output spaces are defined in the style of OpenAI gym.  For example, here's how you would set up a
    PittRulesBrain with the above ruleset that takes two continuous input variables on `(0.0, 1.0)`, and outputs
    ...
    def __init__(self, input_space, output_space, rules, priority_metric, init_mem=[]):
        assert(input_space is not None)
        assert(output_space is not None)
        assert(rules is not None)
        assert(len(rules) > 0)
        assert(priority_metric in PittRulesBrain.PriorityMetric.__members__.values())
        ...
        self.rules = rules
        self.priorities = [self.__priority(r, rule, priority_metric) for (r, rule) in enumerate(rules)]
    ...
    def __priority(self, rule_order, rule, priority_metric):
        ...
        if priority_metric == PittRulesBrain.PriorityMetric.RULE_ORDER:
            ...
            raise ValueError('Unrecognized priority_metric "{0}".'.format(priority_metric))
    ...
    def __match_set(self, input):
        ...
        for (r, rule) in enumerate(self.rules):
            ...
            for c in range(len(all_input)):  # Loop through all conditions
                diff1 = rule[c * 2] - all_input[c]  # TODO Normalize this, in case the possible ranges differ greatly
    ...
    def output(self, input):
        ...
        if best_match_score == 0:
            ...
            highest_priority_rule = min(match_list, key=lambda x: self.priorities[x])
            highest_priority_value = self.priorities[highest_priority_rule]
            match_list = [r for r in match_list if self.priorities[r] <= highest_priority_value]

        # Among the rules set of rules with the highest priority, choose one randomly.
